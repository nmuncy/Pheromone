...
###??? update here
parDir=~/Projects/WiscoPhero			# research directory
derivDir=${parDir}/derivatives
rawDir=${parDir}/rawdata


taskName=PheromoneOlfaction								# name of task (ref Task_step0)
duration=5										# duration of trial (ref Task_step2). Leave empty of duration varies (e.g. 1:1.3 5:0.8)
	...
	for((j=1; j<=$tfRow; j++)); do
		...
		c=0; while [ $c -lt ${#tfArr[@]} ]; do
			...
			if [ $suffix == 1D ]; then
				...
				tmp=${tfArr[$c]##*_}; beh=${tmp%.*}
			fi

			# pull values from row $j of timing file
			#	patchy-patch to account afni wildcard
			sed -n "${j}p" < ${tfPath}/${tfArr[$c]} | grep "\*" >/dev/null 2>&1
			if [ $? != 0 ]; then
